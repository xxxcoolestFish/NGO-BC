"""
experiments/run_parallel.py — 5-node parallel aggregation on MATH.
...
from ngobc.dag import topo_weights
from ngobc.gat_oracle import DagGatOracle
from ngobc.math_dataset import load_math_problems, sample_problems, answers_match
from mas import LLMClient, DAGExecutor, make_parallel_agents, extract_boxed

# Budget tiers (yuan) — scaled for 5 nodes vs 3 nodes (×5/3)
BUDGETS = {
    "ultra_tight": 0.00035, "tight": 0.00050, "low": 0.00080,
    "medium": 0.00120, "high": 0.00200, "generous": 0.00300,
}

DEFAULT_CALIB = {
    "Decomposer": {"sigma": 0.898, "kappa": 0.150, "tau": 13},
    "Solver":     {"sigma": 0.953, "kappa": 0.033, "tau": 56},
    "Aggregator": {"sigma": 0.850, "kappa": 0.020, "tau": 50},
}
...
def run_one(prob, method, budget_label, budget, task_id, calib, oracle):
    ...
    try:
        ...
        answer = executor.get_output("Aggregator") or ""
...
def main():
    ...
    ap.add_argument("--out", default="results/parallel_results.json")
    ap.add_argument("--gat", default="")
    args = ap.parse_args()
